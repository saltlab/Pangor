 ...
 */
public class CBEDestinationScopeAnalysis extends ScopeAnalysis<ClassifierAlert, ClassifierDataSet> {

	/** Stores the possible callback error check repairs. */
	private Set<CallbackErrorCheck> callbackErrorChecks;

	public CBEDestinationScopeAnalysis(ClassifierDataSet dataSet, AnalysisMetaInformation ami) {
		super(dataSet, ami);
			...
			if(node instanceof IfStatement) {
				...
				for(SpecialTypeCheck specialTypeCheck : specialTypeChecks) {
					if(this.parameters.contains(specialTypeCheck.identifier)) {
						CBEDestinationScopeAnalysis.this.registerAlert(node, new CallbackErrorAlert(CBEDestinationScopeAnalysis.this.ami, "[TODO: function name]", "NF_CB", this.function, this.signature, specialTypeCheck.identifier));
						CBEDestinationScopeAnalysis.this.callbackErrorChecks.add(new CallbackErrorCheck(this.scope, this.function, this.signature, specialTypeCheck.identifier, specialTypeCheck.specialType));
					}
				}
